...
export async function login(email, senha) {
  ...
  const { token: refreshToken, expiresAt } = gerarRefreshToken(payload, user.nivel_perfil);

  await salvarRefreshToken({ token: refreshToken, userId: user.id, expiresAt });

  return {accessToken,refreshToken,
    user: {id: user.id,nome: user.nome,email: user.email,nivel_perfil: user.nivel_perfil,},
  };
}
...
export async function refresh(refreshToken) {
  ...
  const tokenSalvo = await buscarRefreshToken(refreshToken);
    if (!tokenSalvo) throw { status: 401, message: "Refresh token não encontrado" };
  ...
  const { token: novoRefreshToken, expiresAt } = gerarRefreshToken(novoPayload, user.nivel_perfil);
  
  await salvarRefreshToken({ token: novoRefreshToken, userId: user.id, expiresAt });
